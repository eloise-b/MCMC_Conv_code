...
ipar = np.array([np.log(0.58e-3), #Dust to gas
                 np.log(2.3e-3), #Gap Depletion
                 np.log(12.7),     #Inner Radius (AU)
                 np.log(25.0),     #Wall Radius (AU)
                 30,               #Inclination
                 138,               #Position Angle
                 0.0,0.0,          #Star position offset (AU)
                 np.log(7000.0),   #Stellar temperature (8250 from Yeon Seok 2015)
...
if (multiprocess):
    ...
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_conv_disk_radmc3d,threads=threads,kwargs={"planet_temp":2000,"temperature":1000,"dist":145.})
